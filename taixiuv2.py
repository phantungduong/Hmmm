...
async def darts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if user_info['balance'] < bet_money:
    remaining_balance = format_currency(user_info['balance'])  # Format số dư hiện tại để hiển thị
    await update.message.reply_text(f'Bạn chỉ còn {remaining_balance}, không thể cược số tiền lớn hơn số dư của bạn ❌',parse_mode='HTML')
    return
...
async def slot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  # Kiểm tra xem người dùng có nhập tiền cược không
  if not context.args:
      await update.message.reply_text("Cú pháp không hợp lệ. Sử dụng: /S [tiền cược]")
      return

  try:
      bet_money = int(context.args[0])  # Chuyển đổi tiền cược từ chuỗi sang số
  except ValueError:
  ...
  # Trừ tiền cược khỏi balance ngay lập tức
  user_info['balance'] -= bet_money
  update_user_info(user_id, user_info['balance'])

  # Gửi icon slot machine
  message = await update.message.reply_dice(emoji=Dice.SLOT_MACHINE)
  await asyncio.sleep(1.6)  # Chờ xúc xắc dừng lại
  ...
  if win_money > 0 and result != 64:
      user_info['balance'] += win_money  # Cộng tiền thắng
      update_user_info(user_id, user_info['balance'])
...
async def taixiu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  with open("data.json", "r+", encoding="utf-8") as file:
      ...
      if user['balance'] < bet_money:
          remaining_balance = format_currency(user['balance'])  # Format số dư hiện tại để hiển thị
          await update.message.reply_text(f'Bạn chỉ còn {remaining_balance}, không thể cược số tiền lớn hơn số dư của bạn ❌',parse_mode='HTML')
          return
      ...
      if (choice == result) or (choice == result_combined):
          winnings = int(bet_money * multiplier)
          user["balance"] += winnings  # Người chơi thắng, cộng tiền
...
async def chanle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if user and money >= chanle_min_bet:
          ...
          message = await update.message.reply_dice()
          await asyncio.sleep(2)  # Đợi kết quả xúc xắc
          total_value = message.dice.value

          result = "Chẵn" if total_value % 2 == 0 else "Lẻ"
          await asyncio.sleep(1)  # Thêm độ trễ nhỏ để tạo ra trải nghiệm người dùng tốt hơn
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if user and money >= doanso_min_bet:
          ...
          if guess == dice_value:
              ...
              user['balance'] += winnings  # Cập nhật số dư sau khi thắng
              ...
              # Và sau đó bạn có thể gửi message_text này như một phản hồi cho người dùng
              await update.message.reply_text(message_text)
          else:
              message_text = (
                "┏━━━━━━━━━━━━━┓\n"
...
async def schedule_summarize(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  while total_time > 0:
      await asyncio.sleep(notify_interval)  # Đợi 10 giây trước khi gửi thông báo
      total_time -= notify_interval  # Giảm thời gian đếm ngược

      # Tính toán thông tin cược
      ...
      # Gửi thông báo với thời gian còn lại và thông tin cược
      if total_time > 0:  # Kiểm tra để không gửi thông báo khi thời gian đã hết
        message = (
  ...
  # Sau khi vòng lặp kết thúc, tức là thời gian đặt cược đã hết
