...
"""
  ...
  if is_user_admin(update.effective_user.id):
      # Thêm các lệnh dành cho admin vào help text
      admin_help_text = """
<b>Lệnh Của Admin</b>
...
"""
      help_text += admin_help_text  # Nối các lệnh admin vào cuối
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  # Tạo và gửi thông điệp bảng xếp hạng
  ...
  try:
      with open('data.json', 'r+', encoding='utf-8') as file:
          ...
          if str(user_id) in users:
              del users[str(user_id)]  # Xóa dữ liệu người dùng

              # Ghi lại dữ liệu vào file
              file.seek(0)
              json.dump(users, file, indent=4, ensure_ascii=False)
              file.truncate()  # Xóa phần dữ liệu cũ không cần thiết
...
async def listcode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  try:
      with open('giftcode.json', 'r', encoding='utf-8') as file:
        ...
        # Xử lý dữ liệu: kiểm tra và xóa giftcode hết lượt sử dụng
      codes_to_remove = [code for code, details in codes.items() if details['uses'] <= 0]
      for code in codes_to_remove:
          del codes[code]

      if codes_to_remove:
          # Ghi lại dữ liệu vào file nếu có sự thay đổi
...
async def chuyentien(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    receiver_username = args[0].lstrip('@')  # Bỏ qua dấu @ ở đầu
    amount = int(args[1])
    ...
    try:
        with open('data.json', 'r+', encoding='utf-8') as file:
            ...
            # Kiểm tra số dư và thực hiện chuyển tiền
...
async def giftcode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  with open('giftcode.json', 'r+', encoding='utf-8') as gift_file:
      ...
      if gift_code in giftcodes and giftcodes[gift_code]['uses'] > 0:
          # Kiểm tra xem user đã sử dụng giftcode này chưa
          with open('data.json', 'r+', encoding='utf-8') as user_file:
              ...
              # Cập nhật data.json và giftcode.json
              user_file.seek(0)
...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if any(user['user_id'] == user_id for user in users):
          ...
          # Thêm người dùng mới vào file
          new_user = {
              "user_id": user_id,
              "user_name": user_name,
              "full_name": full_name,  # Sử dụng full_name thay vì first_name
              "balance": 50000
          }
