...
def remove_banking_request(user_id):
  try:
      ...
      if str(user_id) in data:
          del data[str(user_id)]  # Xoá yêu cầu nạp tiền
          with open("banking.json", "w") as file:
              json.dump(data, file, indent=4)  # Lưu lại file sau khi đã xoá
...
# Hàm cập nhật trạng thái nạp tiền
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
      ...
      if args[0] == "bank:confirm":
          ...
          if update_balance(int(user_id), so_tien):
            ...
            user_name = user.full_name  # Lấy tên người dùng
      ...
      # Xử lý quản trị viên từ chối
      elif args[0] == "bank:deny":
          user_id, so_tien = args[1:]
          user = await context.bot.get_chat(user_id)
          user_name = user.full_name 
          fail_message = f"Yêu cầu nạp {format_currency(so_tien)} của bạn đã bị từ chối"
